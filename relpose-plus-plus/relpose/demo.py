...
def main(image_dir, model_dir, mask_dir, bbox_path, output_path):
    ...
    print("Number of images are ", num_frames)
    ...
    # np.save("T_pred.npy", T_pred.cpu().numpy())

    T_pred = T_pred.cpu().numpy()

    #Read rotation and translation matrices and save images.txt file and intrinsics to cameras.txt

    scene_dir = r"./presentation_experiments/" + output_path[:-5] + r"/"

    idx = 1     #IDX of CAMERA for INTRINSICS

    cameras = read_cameras_binary(scene_dir + r"sparse_colmap/0/cameras.bin")
    print(len(cameras))
    print(cameras.keys())
    print(cameras[1])

    camera_intrinsic = [0, 0, 0, 0, 0, 0, 0, 0, 0]
    camera_intrinsic[0] = cameras[idx].id
    camera_intrinsic[1] = cameras[idx].model
    camera_intrinsic[2] = cameras[idx].width
    camera_intrinsic[3] = cameras[idx].height
    camera_intrinsic[4] = cameras[idx].params[0]
    camera_intrinsic[5] = cameras[idx].params[1]
    camera_intrinsic[6] = cameras[idx].params[2]
    camera_intrinsic[7] = cameras[idx].params[3]
    camera_intrinsic[8] = '\n'

    # R_final = np.load(scene_dir + r"R_final.npy")
    R_final = np.transpose(R_final, (1, 2, 0))
    # T_pred = np.load(scene_dir + r"T_pred.npy")

    final_arr = np.zeros((8, 7))

    for i in range(R_final.shape[2]):
        final_arr[i, :4] = rotmat2qvec(R_final[:, :, i])
        # print(rotmat2qvec(R_final[:, :, i]))
        final_arr[i, 4:] = T_pred[i, :]
        # print(T_pred[i, :])

    final_list = []
    for i in range(final_arr.shape[0]):
        final_list.append(list(final_arr[i, :]))

    image_names = natsorted(os.listdir(scene_dir + r"images"))

    if(".DS_Store" in image_names):
        image_names = image_names.remove(".DS_Store")

    for i in range(len(image_names)):
        final_list[i].insert(0, i+1)
        final_list[i].append(1)
        final_list[i].append(image_names[i])
        # final_list[i].append(r"\n")

    if(os.path.exists(scene_dir + r"sparse/0/") == False):
        os.makedirs(scene_dir + r"sparse/0/")

    camera_path = scene_dir + r"sparse/0/" + r"cameras.txt"
    images_path = scene_dir + r"sparse/0/" + r"images.txt"
    points_path = scene_dir + r"sparse/0/" + r"points3D.txt"

    # print("camera path is", camera_path)
    # print("image path is", images_path)
    # print("points path is", points_path)

    file = open(camera_path,'w')
    for item in camera_intrinsic:
        file.write(str(item))
        file.write(r" ")
    file.close()

    file = open(images_path,'w')
    ...
    R_final = np.transpose(R_final, (2, 0, 1))
